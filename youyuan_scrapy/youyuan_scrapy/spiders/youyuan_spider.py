...
class Youyuan_spider(CrawlSpider):

    name = "Youyuan_spider"
    allowed_domains = ['youyuan.com']
    start_urls = ['http://www.youyuan.com/find/fujian/mm18-0/advance-0-0-0-0-0-0-0/p1/']

    # 主页
    page_links = LinkExtractor(allow=r"/find/fujian/mm18-0/advance-0-0-0-0-0-0-0/p\d+/")
    # 详情页面
    profile_page_lx = LinkExtractor(allow=(r'/\d+-profile/'))

    rules = (
        Rule(page_links,process_links="deal_home_links",follow=True),
        # 匹配个人主页的链接，形成request保存到redis中等待调度，一旦有响应则调用parse_profile_page()回调函数处理，不做继续跟进
        Rule(profile_page_lx,process_links="deal_home_links",callback='parse_profile_page'),
    )

    def deal_home_links(self,links):
        for link in links:
            print(link)
        return links
